...
void main() {
	...
	time_t now;
	struct tm nowlocal;
	now = time(NULL);  //get the time from OS
	...
	hint.sin_addr.S_un.S_addr = INADDR_ANY;     // "INADDR_ANY"  is just Definition of bits in internet address integers.
	...
	//set of our clients & listening ports on the server
	fd_set master;
	//clear set
	FD_ZERO(&master);
	//add client to the set (queue)
			...
			else
			{
				...
				else
				{
					...
					for (int i = 0;i < master.fd_count; i++) {
						...
						if (outsock != sock) {
							ss << "sock #" << sock << ": " << buf << "     " << nowlocal.tm_hour << ":" << nowlocal.tm_min << "\r\n";
							ss << endl;
						}
						else {
							ss << "ME" << ": " << buf << "     " << nowlocal.tm_hour << ":" << nowlocal.tm_min << "\r\n";
							ss << endl;
						}
